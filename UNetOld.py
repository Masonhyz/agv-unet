...
class UNetOld(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        c = cfg["channel"]
        d = cfg["dropout"]
        b = cfg["batch normalization"]

        self.down0 = double_conv_block(3, c)
        self.down1 = downsample_block(c, c * 2, d)
        self.down2 = downsample_block(c * 2, c * 4, d)
        self.down3 = downsample_block(c * 4, c * 8, d)
        self.down4 = downsample_block(c * 8, c * 16, d)

        self.up4 = nn.ConvTranspose2d(c * 16, c * 8, kernel_size=3,
                                      stride=2, padding=1, output_padding=1)
        self.up3 = upsample_block(c * 8, c * 4, d)
        self.up2 = upsample_block(c * 4, c * 2, d)
        self.up1 = upsample_block(c * 2, c, d)
        self.up0 = nn.Sequential(nn.Dropout(d),
